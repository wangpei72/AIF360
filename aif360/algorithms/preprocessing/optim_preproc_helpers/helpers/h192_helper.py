...
def binary_numeric_by_param(df, col_name, param):
    df[col_name] = df[col_name].apply(lambda x: int(x > param))


def binary_numeric_by_mean(df, col_name):
    mean = df[col_name].mean()
    df[col_name] = df[col_name].apply(lambda x: int(x >= mean))
...
if __name__ == '__main__':
    df = pd.read_csv('../../../data/raw/meps/h192.csv', sep=',', header=[0])
    cols_all_initial = df.columns
    print('=========initial cols number: %d' % len(cols_all_initial))
    print(cols_all_initial)
    wtr_cols_txt(cols_all_initial, title='meps16_initial_cols are:\n',
                 file_name='meps16_initial_cols')
    ...
    df = df.rename(columns={'FTSTU53X' : 'FTSTU', 'ACTDTY53' : 'ACTDTY', 'HONRDC53' : 'HONRDC', 'RTHLTH53' : 'RTHLTH',
                              ...
                              'POVCAT16' : 'POVCAT', 'INSCOV16' : 'INSCOV'})  # for all other categorical features, remove values < -1
    ...
    print(XD_features)
    wtr_cols_txt(XD_features, title='meps16_after_pre_cols are:\n',
                 file_name='meps16_after_pre_cols')
    ...
    wtr_cols_txt(list(set(skip_cols_besides) | set(skip_feat)), 'skip cols besides params are \n', 'skip besides')
    ...
    helper.wrt_descrip_txt(df, 'meps16', Y_feat=Y_features, D_feat=D_features,
