...
from aif360.algorithms.preprocessing.optim_preproc_helpers.helpers import helper
...
if __name__ == '__main__':
    df = pd.read_csv('../../../data/raw/home/home.csv', sep=',', header=[0])
    cols_all_initial = df.columns
    print('initial cols number: %d' % len(cols_all_initial))
    print(cols_all_initial)
    wtr_cols_txt(cols_all_initial, title='home_initial_cols are:\n',
                 file_name='home_initial_cols')
    ...
    print(XD_features)
    wtr_cols_txt(XD_features, title='home_after_pre_cols are:\n',
                 file_name='home_after_pre_cols')
    ...
    df.to_csv('df_keep_as_360_home_181.csv')
    helper.wrt_descrip_txt(df, 'home', Y_feat=Y_features, D_feat=D_features,
