...
def clip_numeric_normalize(df, col_name):
    ...
    df[col_name] = df[col_name].apply(lambda x: int(int(x)/ n_pow))
    df.loc[df[col_name] < 1, col_name] = 1
    df.loc[df[col_name] > 9, col_name] = 9
...
def work_flow(df, y_labels, skip_feat = None, binary_0_feat=None,  age_feat=None, binary_avg_feat=None):
    for i in df.columns:
        if i == 'Unnamed: 0':
            continue
        if y_labels is not None and i in y_labels:
            continue
        if skip_feat is not None and i in skip_feat:
            continue
        if binary_0_feat is not None and i in binary_0_feat:
